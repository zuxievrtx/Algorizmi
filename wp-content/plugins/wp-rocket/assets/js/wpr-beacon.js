  ...
  var BeaconLcp = class {
    ...
    _generateLcpCandidates(count) {
      ...
      const topCandidates = potentialCandidates
        .map((element) => {
          if (
            "img" === element.nodeName.toLowerCase() &&
        ...
        .filter((item) => {
          return (
            item.rect.width > 0 &&
            item.rect.height > 0 &&
            Utils_default.isIntersecting(item.rect)
          );
        })
        .map((item) => ({
          item,
    ...
    _getElementInfo(element) {
      ...
      const css_bg_url_rgx = /url\(\s*?['"]?\s*?(.+?)\s*?["']?\s*?\)/gi;
      if (nodeName === "img" && element.srcset) {
        ...
        element_info.sources = Array.from(
        ...
        if (
          !full_bg_prop ||
          full_bg_prop === "" ||
          full_bg_prop.includes("data:image")
        ) {
          return null;
        }
        const matches = [...full_bg_prop.matchAll(css_bg_url_rgx)];
        element_info.bg_set = matches.map((m) =>
          m[1] ? { src: m[1].trim() + (m[2] ? " " + m[2].trim() : "") } : {}
        );
    ...
    _initWithFirstElementWithInfo(elements) {
      const firstElementWithInfo = elements.find(
        (item) => item.elementInfo !== null
      );
  ...
  var BeaconLrc = class {
    ...
    _getLazyRenderElements() {
      ...
      const validElements = Array.from(elements).filter(
        (element) => !this._skipElement(element)
      );
    ...
    _getElementDistance(element) {
      ...
      return Math.max(
        0,
        rect.top + scrollTop - Utils_default.getScreenHeight()
      );
    }
    _skipElement(element) {
      const skipStrings = this.config.skipStrings || ["memex"];
      if (!element || !element.id) return false;
      return skipStrings.some((str) =>
        element.id.toLowerCase().includes(str.toLowerCase())
      );
    }
    ...
    _checkLcrConflict(element) {
      ...
      const negativeMargins = validMargins.some(
        (margin) => parseFloat(computedStyle[margin]) < 0
      );
      const currentElementConflicts =
        negativeMargins ||
      ...
      Array.from(element.children).forEach((child) => {
        ...
        const childNegativeMargins = validMargins2.some(
          (margin) => parseFloat(childStyle[margin]) < 0
        );
        const childConflicts =
          childNegativeMargins ||
      ...
      elements.forEach(({ element, depth, distance, hash }) => {
        ...
        if (conflicts.length > 0) {
          this.logger.logMessage(
            "Skipping element due to conflicts:",
            conflicts
          );
          return;
        }
        const can_push_hash =
          ...
          this._getElementDistance(element.parentElement) <
            this.config.lrc_threshold &&
          distance >= this.config.lrc_threshold;
        const color = can_push_hash ? "green" : distance === 0 ? "red" : "";
        this.logger.logColoredMessage(
          `${"	".repeat(depth)}${
            element.tagName
          } (Depth: ${depth}, Distance from viewport bottom: ${distance}px)`,
          color
        );
        this.logger.logColoredMessage(
          `${"	".repeat(depth)}Location hash: ${hash}`,
          color
        );
        this.logger.logColoredMessage(
          `${"	".repeat(depth)}Dimensions Client Height: ${
    ...
    _getElementXPath(element) {
      ...
      return `${this._getElementXPath(
        element.parentNode
      )}/${element.nodeName.toLowerCase()}[${position}]`;
    }
  ...
  var BeaconManager = class {
    ...
    async init() {
      ...
      const shouldGenerateLcp =
        this.config.status.atf &&
        (isGeneratedBefore === false || isGeneratedBefore.lcp === false);
      const shouldGeneratelrc =
        this.config.status.lrc &&
        (isGeneratedBefore === false || isGeneratedBefore.lrc === false);
    ...
    async _isValidPreconditions() {
      ...
      if (
        Utils_default.isNotValidScreensize(this.config.is_mobile, threshold)
      ) {
        this.logger.logMessage(
    ...
    _finalize() {
      const beaconscript = document.querySelector(
